...
| 18 | Distribution | **LIVE via public Homebrew tap; source repo is public** _(updated 2026-07-02)_. Coda ships as a notarized `.app` through the public tap [`IsaacArnold/homebrew-coda`](https://github.com/IsaacArnold/homebrew-coda) (`brew tap` → `brew trust` → `brew install --cask coda`); the source repo [`IsaacArnold/coda`](https://github.com/IsaacArnold/coda) is now **public** too (MIT LICENSE is the no-warranty shield). Notarized under a personal Apple Developer ID; `scripts/release.sh` tests → builds → notarizes → publishes. Background on the portable-bundle work: **Portable signed `.app`, notarization-ready** _(updated 2026-06-30)_. `scripts/make-app.sh` builds a `.app`+dmg whose assets ship flat in `Contents/Resources` — this **fixed a latent bug** where the old layout (SwiftPM resource bundles in `Contents/MacOS`) crashed on launch on any machine but the build host, because the generated accessor fell back to an absolute `.build` path. Default build is ad-hoc sealed (Gatekeeper blocks recipients once). Set `DEVELOPER_ID_APP` (+ `NOTARY_PROFILE`, or `NOTARY_APPLE_ID`/`NOTARY_TEAM_ID`/`NOTARY_PASSWORD`) to seal with the Hardened Runtime — **zero entitlements** (not sandboxed; child-process spawn is allowed by default) — then notarize + staple. **For now:** hand colleagues the unsigned dmg via AirDrop/USB (no quarantine, no Gatekeeper bypass). **Escalate** to the notarized path (one-time $99/yr Apple Developer cert, then just flip the env vars) when a managed work Mac blocks unsigned apps. SwiftTerm's Metal-shader bundle is intentionally not shipped (Coda uses the AppKit renderer, never the Metal path). |
